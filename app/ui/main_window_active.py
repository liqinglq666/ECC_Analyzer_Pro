...
from PySide6.QtWidgets import QFileDialog, QMessageBox, QApplication, QHeaderView, QSplitter
...
class MainWindow(_BaseMainWindow):
    ...
    说明
    ----
    app.ui.main_window.MainWindow 保留为基础窗口实现，本类在其上集中维护
    当前版本真正对用户生效的 UI 行为：

    1. Basic 表格显式区分 ε_peak 与 ε_u；
    2. 统计图使用同一套指标标签；
    3. 导出函数读取 DataExporter 的 True/False 状态，避免失败时误报成功；
    4. 左侧数据表区域加宽，适配 Basic 视图新增的第五个指标列。
    """
