...
class MaterialConstants:
    """
    ECC 物理常数与全局配置管理器 V2.1 (Full JSON Sync Edition)
    - 核心功能：管理物理参数、算法阈值及绘图样式。
    - 健壮性：增加了类型强制转换和默认值回滚机制。
    - 持久化：完美映射全量 JSON 高级配置，确保读写不丢失。
    """
    ...
    ELASTIC_STRAIN_LIMIT: float = 0.0005  # 弹性应变硬性限制
    INITIAL_MODULUS_SEARCH_LIMIT: float = 0.15 # 初始模量搜索范围
    INITIAL_MODULUS_PERCENTILE: float = 0.10 # 初始模量取前 10%

    # ==========================================
    # 3. 开裂判定 (Cracking) - 多重耦合阈值
    # ==========================================
    CRACK_TOLERANCE_BASE: float = 0.05  # 偏离线性的容差 (MPa)
    CRACK_TOLERANCE_RATIO: float = 0.01 # 偏离比例容差
    CRACK_STIFFNESS_CONSTRAINT: float = 0.85 # 刚度衰减约束 (85% E_init)
    CRACK_MIN_STRESS_RATIO: float = 0.05 # 最小触发应力比例 (底噪过滤)

    # ==========================================
    # 4. 极限状态 (Ultimate)
    # ==========================================
    ULTIMATE_STRAIN_RATIO: float = 0.85  # 峰后下降至 85% 视为破坏
    ZERO_STRESS_THRESHOLD: float = 0.01  # 零应力判定阈值

    # ==========================================
    # 5. 数据处理 (Processing)
    # ==========================================
    PEAK_SMOOTH_WINDOW: int = 5  # 寻峰平滑窗口
    SMOOTH_WINDOW: int = 15  # 全局平滑窗口 (必须是奇数)
    SMOOTH_POLY: int = 3  # 多项式阶数

    # ==========================================
    # 6. 绘图样式 (Visualization)
    # ==========================================
    STYLE_COLOR_RAW: str = "#ffaa00"  # 原始曲线颜色
    STYLE_COLOR_SMOOTH: str = "#ffaa00" # 平滑曲线颜色
    STYLE_LINE_WIDTH: float = 1.5  # 默认线宽
    STYLE_RAW_ALPHA: float = 0.6  # 原始数据透明度
